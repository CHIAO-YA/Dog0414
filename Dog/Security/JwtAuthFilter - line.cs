...
{
    public class JwtAuthFilterline : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            else
            {
                try
                {
                    ...
                    var jwtObject = GetlineToken(request.Headers.Authorization.Parameter);

                    // 檢查有效期限是否過期，如 JwtToken 過期，需導引重新登入
                    if (IslineTokenExpired(jwtObject["Exp"].ToString()))
                    {
        ...
        //將 Token 解密取得夾帶的資料
        public static Dictionary<string, object> GetlineToken(string token)
        {
            return JWT.Decode<Dictionary<string, object>>(token, Encoding.UTF8.GetBytes(secretKey), JwsAlgorithm.HS512);
        }

        // 驗證 Token 是否過期
        public bool IslineTokenExpired(string dateTime)
        {
