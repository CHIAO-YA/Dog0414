...
namespace Dog.Controllers
{
    public class UserfirstController : ApiController
    {
        ...
        private string GetStatusText(OrderStatus? status)// 訂單狀態
        {
            switch (status)
            {
                case OrderStatus.未完成: return "未完成";
                case OrderStatus.前往中: return "前往中";
                case OrderStatus.已完成: return "已完成";
                case OrderStatus.異常回報: return "異常回報";
        ...
        public IHttpActionResult GetUserOrder(int UsersID)
        {
            ...
            // 獲取用戶的所有訂單，排除未付款訂單
            var allUserOrders = db.Orders.Include("OrderDetails").Include("Plan").Where(o => o.UsersID == UsersID && o.PaymentStatus != PaymentStatus.未付款).ToList();
            // 查詢該用戶的訂單，只要訂單詳情中有任何一筆是「未完成」或「前往中」狀態，就視為有效訂單
            // 並且確保訂單狀態不是未付款
            var ValidOrders = db.Orders.Include("OrderDetails").Include("Plan").Where(o => o.UsersID == UsersID && o.PaymentStatus != PaymentStatus.未付款 && o.OrderDetails.Any(od => od.OrderStatus == OrderStatus.未完成 || od.OrderStatus == OrderStatus.前往中)).ToList();            // 取得這些訂單的所有照片，避免多次查詢// 先獲取訂單 ID 列表// 然後使用這個列表來查詢照片
            var orderIds = ValidOrders.Select(o => o.OrdersID).ToList();
        ...
        public IHttpActionResult GetUserEndOrder(int UsersID)
        {
            ...
            if (!db.Users.Any(u => u.UsersID == UsersID)) { return NotFound(); }
            // 獲取用戶的所有訂單
            var allUserOrders = db.Orders.Include("OrderDetails").Include("Plan").Where(o => o.UsersID == UsersID).ToList();

            // 過濾出已結束的訂單：
            // 1. 所有訂單詳情都是「已完成」或「已取消」
            // 2. 或者所有服務日期都是過去日期
            var today = DateTime.Today;
            var EndOrders = allUserOrders.Where(o => o.OrderDetails.All(od => od.OrderStatus == OrderStatus.已完成 || od.OrderStatus == OrderStatus.已取消) || o.OrderDetails.All(od => od.ServiceDate.Date < today)).ToList();
            // 取得這些訂單的所有照片，避免多次查詢
            if (!EndOrders.Any())
            {
                return Ok(new
        ...
        {
            return orderDetails.Count(od => (od.OrderStatus == OrderStatus.未完成 || od.OrderStatus == OrderStatus.前往中) && od.ServiceDate > DateTime.Now);
        ...
        public async Task<IHttpActionResult> CreateOrder()
        {
            ...
            // 計算訂單總金額並更新
            CalculateTotalAmount(orders.OrdersID);
            ...
            foreach (var serviceDate in serviceDates)
            {
                var orderDetail = new OrderDetails
                {
                    ...
                    OrderStatus = OrderStatus.未完成, // 設置初始狀態
        ...
        [HttpPost]
        [Route("POST/user/orders/11")]//新增訂單
